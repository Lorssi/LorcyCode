...
def _extract_archive(archive_path: str | Path, target_path: Path) -> bool:
    """安全解压压缩包，过滤路径穿越攻击"""
...
def validate_skill_package(archive_path: str) -> dict | None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 解析元数据
            metadata = loader._parse_skill_metadata(skill_md)
            if not metadata or not _is_safe_skill_name(metadata.name):
                return None
...
def install_skill(archive_path: str, install_path: Path) -> bool:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 先完整复制到同一文件系统中的临时目录，成功后再替换旧版本。
            with tempfile.TemporaryDirectory(
                prefix=f".{skill_name}.", dir=install_path
            ) as staging_root:
                staged_dir = Path(staging_root) / skill_name
                shutil.copytree(skill_source_dir, staged_dir)
